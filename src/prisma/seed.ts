...
import { UserRole, CompanyStatus, SusuPlanType, CollectionStatus, TransactionType } from '../types/enums';
...
async function main() {
    ...
    for (let j = 0; j < accountCount; j++) {
      ...
      const account = await prisma.susuAccount.create({
        data: {
          ...
          startDate: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000), // Random date in last 30 days
          isActive: true,
        },
      });
  ...
  for (let dayOffset = 0; dayOffset < 14; dayOffset++) {
    const collectionDate = new Date(fourteenDaysAgo.getTime() + dayOffset * 24 * 60 * 60 * 1000);
      ...
      if (Math.random() < 0.8) {
        ...
        const collection = await prisma.collection.create({
          data: {
            ...
            status: status,
            notes: status === CollectionStatus.MISSED ? 'Customer not available' : 
                   status === CollectionStatus.PARTIAL ? 'Partial payment made' : 
